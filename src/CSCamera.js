  ...
  if (captureAudio) {
    if (Platform.OS === 'ios') {
      ...
      if (await CameraManager.checkIfRecordAudioPermissionsAreDefined()) {
        ...
        console.warn(
          `The 'captureAudio' property set on CSCamera instance but 'RECORD_AUDIO' permissions not defined in the applications 'AndroidManifest.xml'. ` +
...
const CSCamera = requireNativeComponent('CSCamera', Camera, {
  nativeOnly: {
